...
A real homebrew ROM already loads, executes, renders, and accepts input on actual hardware. The first measured complete frame path ran at about 5 FPS. After device-side profiling and several renderer and CPU experiments, the best render-every-frame path reaches 13 FPS, fixed FS2 reaches 18 guest / 9 display FPS, and the CPU-only path reaches 28 FPS.
...
- ROW8, fixed frame-skip and layered DIV v2 benchmark builds;
...
This is not a finished emulator release. There is no sound, save RAM, ROM chooser, or general compatibility claim. Fixed frame skip and current-workload profiling are now complete; the next practical step is a narrow same-binary 16-bit x86 assembly experiment.
